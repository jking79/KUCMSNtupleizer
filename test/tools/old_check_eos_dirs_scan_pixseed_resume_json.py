...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Older text-style .live.txt files cannot be resumed and are restarted."
        ),
    )
    parser.add_argument(
    ...
    for idir, eos_dir in enumerate(scan_dirs, start=1):
        ...
        if live_data is not None:
            ...
            if args.skip_completed and os.path.exists(paths["live"]):
                print_status(
                    f"Found existing .live.txt but it is not a compatible JSON checkpoint; restarting this directory: {paths['live']}"
                )
