...
void KUCMSAodSkimmer::processPhotons(){
    ...
    for( uInt it = 0; it < nPhotons; it++ ){
		...
		//if( isEleVeto ) nEleVeto++;
        auto rhids = (*Photon_rhIds)[it];
        uInt nrh = rhids.size();

		bool phoskip = isExcluded || hasEleVeto || not isEB;
		if( phoskip ) continue;		
...
void KUCMSAodSkimmer::processElectrons(){
    ...
    float nEpsidsolo = 0; 
    for( uInt it = 0; it < nElectrons; it++ ){
    
		auto eta = (*Electron_eta)[it];
    	auto phi = (*Electron_phi)[it];
		auto esid = getLeadRhID( (*Electron_rhIds)[it] );
		auto eletime = (*Electron_seedTOFTime)[it];
    	...
    	for( uInt it2 = 0; it2 < nPhotons; it2++ ){
        	auto eta2 = (*Photon_eta)[it2];
        	auto phi2 = (*Photon_phi)[it2];
        	auto elephodr = hypo( eta-eta2, dPhi(phi,phi2) );
        	//std::cout << " -- Eta : " << eta-eta2 << " Phi : " << dPhi(phi,phi2) << " dr : " << pho2dr << std::endl;
        	if( elephodr < elePhoIsoDr ) elePhoIsoDr = elephodr;
			auto psid = getLeadRhID( (*Photon_rhIds)[it2] );
