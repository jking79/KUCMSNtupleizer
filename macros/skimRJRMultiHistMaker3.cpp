...
// ( minJetsPer, nphos, maxSelPhos, rjrcut2, metcut )
	...
	if( numPhos > 0 ) hist1d[2]->Fill(9,fillwt);
    if( numPhos > 1 ) hist1d[2]->Fill(10,fillwt);
    if( numPhos > 2 ) hist1d[2]->Fill(11,fillwt);
	if( numPhos < cutvb ) continue;
	hist1d[2]->Fill(12,fillwt);

    if( (*rjrNJetsJa)[cs] > 0 && (*rjrNJetsJb)[cs] > 0 ) hist1d[2]->Fill(13,fillwt);
    if( (*rjrNJetsJa)[cs] > 1 && (*rjrNJetsJb)[cs] > 1 ) hist1d[2]->Fill(14,fillwt);
    //if( (*rjrNJetsJa)[cs] > 3 && (*rjrNJetsJb)[cs] > 3 ) hist1d[2]->Fill(18,fillwt);

	if( (*rjrNJetsJa)[cs] < cutv || (*rjrNJetsJb)[cs] < cutv ) continue;
    hist1d[2]->Fill(15,fillwt);
	...
	if( (*rjrAX2NQSum)[cs] > 0.1 ) hist1d[2]->Fill(16,fillwt);
    if( (*rjrAX2NQSum)[cs] > 0.2 ) hist1d[2]->Fill(17,fillwt);
    if( (*rjrAX2NQSum)[cs] > 0.3 ) hist1d[2]->Fill(18,fillwt);
    if( (*rjrAX2NQSum)[cs] > 0.4 ) hist1d[2]->Fill(19,fillwt);
    if( (*rjrAX2NQSum)[cs] > 0.5 ) hist1d[2]->Fill(20,fillwt);
	if( (*rjrAX2NQSum)[cs] <= cutvc ) continue;
	hist1d[2]->Fill(21,fillwt);

    if( rjrNVSum > 0.1 ) hist1d[2]->Fill(22,fillwt);
    if( rjrNVSum > 0.15 ) hist1d[2]->Fill(23,fillwt);
    if( rjrNVSum > 0.2 ) hist1d[2]->Fill(24,fillwt);
    if( rjrNVSum <= cutvd ) continue;
 	hist1d[2]->Fill(25,fillwt);

    if( (*rjrASMass)[cs] > 100 ) hist1d[2]->Fill(26,fillwt);
    if( (*rjrASMass)[cs] > 1000 ) hist1d[2]->Fill(27,fillwt);
    if( (*rjrASMass)[cs] > 2000 ) hist1d[2]->Fill(28,fillwt);
    if( (*rjrASMass)[cs] <= cutve ) continue;
	hist1d[2]->Fill(29,fillwt);	

    if( std::abs( rjrVDiff ) < 0.6 ) hist1d[2]->Fill(30,fillwt);
    if( std::abs( rjrVDiff ) < 0.4 ) hist1d[2]->Fill(31,fillwt);
	if( std::abs( rjrVDiff ) >= cutvf ) continue;
	hist1d[2]->Fill(32,fillwt);
...
int main ( int argc, char *argv[] ){
    ...
    //else {
                ...
                //std::string infilename = infilenameWJLNu;
				//std::string htitles = htitlewjlnu;
                std::string infilename = infilenameJ;
                std::string htitles = htitleJ5;
