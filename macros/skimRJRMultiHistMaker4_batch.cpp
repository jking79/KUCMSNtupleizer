...
void HistMaker::histMaker( std::string indir, std::string infilelist, std::string outfilename, std::string htitle, 
		int cut, float va, float vb, float vc, float vd, float ve, float vf ){
	...
	const std::string ofnending = "_RjrSkim_v24_cfHists.root";
...
//------------------------------------------------------------------------------------
    ...
    cfbin++;
    //hist1d[2]->Fill(cfbin,fillwt);
    //if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"jabc");
    ...
    cfbin++;
    if( (*rjrASMass)[cs] > 1000 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"M10d");
    cfbin++;
    if( (*rjrASMass)[cs] > 1500 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"M15d");
    cfbin++;
    if( (*rjrASMass)[cs] > 2000 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"M20d");
    cfbin++;
    if( (*rjrASMass)[cs] > 2500 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"M25d");
    cfbin++;
    if( (*rjrASMass)[cs] > 3000 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"M30d");
    cfbin++;
    if( (*rjrASMass)[cs] > 3500 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"M35d");
    cfbin++;
    if( (*rjrASMass)[cs] > 4000 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"M40d");
    if( (*rjrASMass)[cs] <= cutve ) continue;
    cfbin++;
    //hist1d[2]->Fill(cfbin,fillwt);
    //if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Mc");

    cfbin++;
	if( (*rjrAX2NQSum)[cs] > 0.6 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"R6");
    cfbin++;
    if( (*rjrAX2NQSum)[cs] > 0.5 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"R5");
    cfbin++;
    if( (*rjrAX2NQSum)[cs] > 0.4 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"R4");
    cfbin++;
    if( (*rjrAX2NQSum)[cs] > 0.3 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"R3");
    cfbin++;
    if( (*rjrAX2NQSum)[cs] > 0.2 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"R2");
	if( (*rjrAX2NQSum)[cs] <= cutvc ) continue;
    cfbin++;
	//hist1d[2]->Fill(cfbin,fillwt);
    //if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Rc");

    cfbin++;
    if( rjrNVSum > 0.1 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Rv1");
    cfbin++;
    if( rjrNVSum > 0.2 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Rv2");
    cfbin++;
    if( rjrNVSum > 0.3 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Rv3");
    cfbin++;
    if( rjrNVSum > 0.4 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Rv4");
    cfbin++;
    if( rjrNVSum > 0.5 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Rv5");
    cfbin++;
    if( rjrNVSum > 0.6 ) hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"Rv6");
    if( rjrNVSum <= cutvd ) continue;
    cfbin++;

    cfbin++;
 	hist1d[2]->Fill(cfbin,fillwt);
    if( fl ) hist1d[2]->GetXaxis()->SetBinLabel(cfbin,"FCut");
...
void HistMaker::initHists( std::string ht ){
    ...
    hist1d[2] = new TH1D("cutflow", addstr(ht,"_cutFlow").c_str(), 36, 0.5, 36.5);
...
int main ( int argc, char *argv[] ){
    ...
    //else {
				...
				int as = ax;
				//for( int vd = 0; vd < 2; vd++ ){// vis diff - dropped		
				int vd = ( ax < 2 ) ? 0 : 1;//RV		

					//float cutc = ( float(ax) + 2.f )/10.f;//R
                    float cutc = ( np == 1 ) ? 0.4 : 0.2;//R
					float cutd = ( nj == 1 ) ? 0.2 : 0.2;//float(nv) * 0.2;//RV
					float cute = ( ax == 1 ) ? 1500 : 1000;//as * 2000;//M
					float cutf = ( vd == 0 ) ? 0.6 : 0.4;//D - dropped

					std::string isoline = "mSP" + std::to_string( minSelPhos ) + "_"; 
					isoline += "mj" + std::to_string( minJetsPer ) + "_";
					isoline += "rp" + std::to_string( nRjrPhos ) + "_" ;
					isoline += "M" + std::to_string( int(cute) ) + "_";
					isoline += "R0p" + std::to_string( int(cutc*10) ) + "_";
					isoline += "Rv0p" + std::to_string( int(cutd*10) ) + "_";
