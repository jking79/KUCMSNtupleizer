...
//---------fitter.MakeTimeFits()------ run script substances class with <TimeFitter fitter;> then runs <fitter.MakeTimeFits();> -------------------
  ...
  ResultsMap["chi2ndf"]  = new TH1F(chi2ndfName.c_str(),chi2ndfName.c_str(),fXBins.size()-1,xbins);
  ...
  ResultsMap["chi2prob"] = new TH1F(chi2probName.c_str(),chi2probTitle.c_str(),fXBins.size()-1,xbins);
  ...
  ResultsMap["mu"] = new TH1F(muName.c_str(),muTitle.c_str(),fXBins.size()-1,xbins);
  ResultsMap["mu"]->Sumw2();
  ...
  ResultsMap["sigma"] = new TH1F(sigName.c_str(),sigTitle.c_str(),fXBins.size()-1,xbins);
  ...
  ResultsMap["occ"] = new TH1F(occName.c_str(),occTitle.c_str(),fXBins.size()-1,xbins);
  ...
  ResultsMap["rms"] = new TH1F(rmsName.c_str(),rmsTitle.c_str(),fXBins.size()-1,xbins);
  ...
  for (auto ibinX = 1; ibinX < fNBinsX; ibinX++){ 
	...
	//std::cout << "Getting ErrorHists "<< std::endl;
	...
	float err = 0.f;  ///  <<<<<  --------------   (TH2F*)fInFile->Get(errHistName.c_str()); broken with new bins - fix !!!!!!
