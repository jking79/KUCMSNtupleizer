...
class KUCMSEcalRecHitObjectMini : public KUCMSObjectBase {

    public:

    // use base class constructor
    KUCMSEcalRecHitObjectMini( const edm::ParameterSet& iConfig );
	~KUCMSEcalRecHitObjectMini(){};
...
KUCMSEcalRecHitObjectMini::KUCMSEcalRecHitObjectMini( const edm::ParameterSet& iConfig ):
...
void KUCMSEcalRecHitObjectMini::LoadRecHitTokens( edm::EDGetTokenT<recHitCol> recHitsEBToken, edm::EDGetTokenT<recHitCol> recHitsEEToken ){
...
void KUCMSEcalRecHitObjectMini::LoadSCTokens( edm::EDGetTokenT<reco::SuperClusterCollection> scToken, edm::EDGetTokenT<reco::SuperClusterCollection> ootScToken ){

	scToken_ = scToken;
	ootScToken_ = ootScToken;

}//<<>>void KUCMSEcalRecHitObjectMini::LoadSCTokens( edm::EDGetTokenT<reco::SuperClusterCollection>*

void KUCMSEcalRecHitObjectMini::LoadClusterTokens( edm::EDGetTokenT<std::vector<reco::CaloCluster>> ccToken ){

	ccToken_ = ccToken;

}//<<>>void KUCMSEcalRecHitObjectMini::LoadClusterTokens( edm::EDGetTokenT<std::vector<reco::CaloCluster>>* ccToken )

void KUCMSEcalRecHitObjectMini::LoadSCTokens( edm::EDGetTokenT<reco::SuperClusterCollection> otherScToken ){

    otherScToken_ = otherScToken;

}//<<>>void KUCMSEcalRecHitObjectMini::LoadSCTokens( edm::EDGetTokenT<reco::SuperClusterCollection>*

void KUCMSEcalRecHitObjectMini::InitObject( TTree* fOutTree ){
...
void KUCMSEcalRecHitObjectMini::LoadEvent( const edm::Event& iEvent, const edm::EventSetup& iSetup, ItemManager<float>& geVar ){
...
*/
    ...
    for( const auto &supclstr : *otherSuperCluster_ ){
        if(supclstr.energy() < cfPrm("minSCE")) continue;
        bool skip = false;
        const auto & pSeedDetId = supclstr.seed()->seed(); // get seed detid 
...
void KUCMSEcalRecHitObjectMini::ProcessEvent( ItemManager<float>& geVar ){}

void KUCMSEcalRecHitObjectMini::PostProcessEvent( ItemManager<float>& geVar ){
...
void KUCMSEcalRecHitObjectMini::EndJobs(){}

void KUCMSEcalRecHitObjectMini::setRecHitUsed( rhIdGroup idgroup){
...
void KUCMSEcalRecHitObjectMini::setRecHitUsed( rhIdGroup idgroup, bool isOOT, float sce, std::vector<float> fracs ){
...
float KUCMSEcalRecHitObjectMini::getSuperClusterSeedTime( reco::SuperClusterRef supercluster ){
...
}//<<>>float KUCMSEcalRecHitObjectMini::getSuperClusterIndex( reco::SuperCluster supercluster )

int KUCMSEcalRecHitObjectMini::getSuperClusterIndex( const reco::SuperCluster* supercluster, int objectID, int objectIdx ){
...
bool KUCMSEcalRecHitObjectMini::reduceRhGrps( std::vector<rhGroup> & x ){
...
}//>>>>std::vector<rhGroup> KUCMSEcalRecHitObjectMini::reduceRhGrps(const std::vector<rhGroup> x)

void KUCMSEcalRecHitObjectMini::mrgRhGrp( rhGroup & x, rhGroup & y ){
...
}//>>>>rhGroup KUCMSEcalRecHitObjectMini::mrgRhGrp(const rhGroup x, const rhGroup y)

float KUCMSEcalRecHitObjectMini::getLeadTofTime( rhGroup recHits, double vtxX, double vtxY, double vtxZ ){
...
float KUCMSEcalRecHitObjectMini::getSeedTofTime( reco::SuperCluster sprclstr, double vtxX, double vtxY, double vtxZ ){
...
}//>>>>float  KUCMSEcalRecHitObjectMini::getSeedTofTime( rhGroup recHits, double vtxX, double vtxY, double vtxZ )

std::vector<float> KUCMSEcalRecHitObjectMini::getLeadTofRhTime( rhGroup recHits, double vtxX, double vtxY, double vtxZ ){
...
EcalRecHit KUCMSEcalRecHitObjectMini::getLeadRh( rhGroup recHits ){
...
float KUCMSEcalRecHitObjectMini::getRhTOF( EcalRecHit rechit, double vtxX, double vtxY, double vtxZ ){
...
std::vector<float> KUCMSEcalRecHitObjectMini::getRhTofTime( rhGroup recHits, double vtxX, double vtxY, double vtxZ ){
...
rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( const reco::CaloCluster basicCluster, float minenr ){
...
}////rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( const reco::CaloCluster basicCluster, float minenr = 0.0 )

rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( const scGroup superClstrGrp, float minenr, std::vector<float> phEnergy, std::vector<float> phDr, float phEnMax ){
...
rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( const scGroup superClusterGroup, float minenr ){
...
}//>>>>rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( const scGroup superClusterGroup, float minenr = 0.0 )

std::vector<float> KUCMSEcalRecHitObjectMini::getFractionList( const scGroup superClusterGroup, rhGroup recHits ){
...
rhIdGroup KUCMSEcalRecHitObjectMini::getRhRawIdList( const scGroup superClusterGroup ){
    ...
    for ( const auto &superCluster : superClusterGroup ){
        ...
        for( uInt iHAF = 0; iHAF < nHAF; iHAF++ ){
            ...
            for (const auto &recHit : frechits ){
                const auto recHitId = recHit.detid();
                const uInt rhRawId = recHitId.rawId();
                if( rhRawId == rawId ){ list.push_back(rhRawId); break; } //std::cout << " ---- Found " <<  std::endl; break; }
            }//<<>>for (const auto &recHit : recHits ){
        }//<<>>for( uInt iHAF = 0; iHAF < nHAF; iHAF++ )
    }//<<>>for ( const auto superCluster : superClusterGroup )
    //std::cout << " -------------------------------------------------------------- " << std::endl;
...
}//<<>>std::vector<float> KUCMSEcalRecHitObjectMini::getRhRawIdList( const scGroup superClusterGroup )

std::vector<float> KUCMSEcalRecHitObjectMini::getMissFractionList( const scGroup superClusterGroup ){
...
}//<<>>std::vector<float> KUCMSEcalRecHitObjectMini::getMissFractionList( const scGroup superClusterGroup, rhGroup recHits )

rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( uInt detid ){
...
float KUCMSEcalRecHitObjectMini::getRecHitEnergy( const uInt id ){
...
float KUCMSEcalRecHitObjectMini::getRecHitTime( const uInt id ){
...
rhGroup KUCMSEcalRecHitObjectMini::getRHGroup(){
...
rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( float eta, float phi, float drmin, float minenr ){
...
}//>>>>rhGroup KUCMSEcalRecHitObjectMini::getRHGroup( float eta, float phi, float drmin, float minenr = 0.0 )

Cluster2ndMoments KUCMSEcalRecHitObjectMini::getCluster2ndMoments( const reco::SuperCluster* scptr ){
...
}//<<>>Cluster2ndMoments KUCMSEcalRecHitObjectMini::getCluster2ndMoments( reco::SuperCluster* scptr )


std::array<float, 3> KUCMSEcalRecHitObjectMini::getCovariances( const reco::SuperCluster* scptr ){
...
}//<<>>std::array<float, 3> KUCMSEcalRecHitObjectMini::getCovariances( reco::SuperCluster* scptr )
