...
- make sure all the external files necessary to the skimmer are in the config folder and this is current with the config.tgz tarball in the `condor/` directory
	...
	- if you need to update teh config.tgz tarball, you can use the command `make configtar` from the base skimmer directory THIS SHOULD BE DONE EVERY TIME THE EXECUTABLE IS RECOMPILED or any of the external input files change
- the script generator is set up to run over either `master` list of file lists or a single file list for signal only right now
	...
	- ex) `python generateSubmission.py -i GJets --year 2018 -o v41` will create a single condor submission script for each GJets HT slice sample 
