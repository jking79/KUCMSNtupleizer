...
int main ( int argc, char *argv[] ){
	...
	for(int i = 0; i < argc; i++){
		...
		if(strncmp(argv[i], "--hasGenInfo", 12) == 0){
			hasGenInfo = true;
		}
		if(strncmp(argv[i], "--genSigPerfect", 15) == 0){
			genSigPerfect = true;
		}
		if(strncmp(argv[i], "--noSVorPho", 11) == 0){
			noSVorPho = true;
		}
                if(strncmp(argv[i],"--dataSetKey", 12) == 0){
                        i++;
                        key = string(argv[i]);
                }
                if(strncmp(argv[i],"--xsec", 6) == 0){
                        i++;
                        xsec = std::stof(argv[i]);
                }
                if(strncmp(argv[i],"--gluinoMass", 12) == 0){
                        i++;
                        glumass = std::stof(argv[i]);
                }
                if(strncmp(argv[i],"--N2Mass", 8) == 0){
                        i++;
                        n2mass = std::stof(argv[i]);
                }
                if(strncmp(argv[i],"--timeCaliTag", 13) == 0){
                        i++;
                        ttag = string(argv[i]);
                }
                if(strncmp(argv[i],"--MCweight", 10) == 0){
                        i++;
                        mcw = std::stof(argv[i]);
                }
	}
